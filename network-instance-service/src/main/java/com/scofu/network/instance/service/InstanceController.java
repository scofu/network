...
final class InstanceController implements Feature {
        ...
        new PodBuilder()
            ...
            .withVolumeMounts(
                new VolumeMountBuilder()
                    ...
                    .withMountPath("/opt/shareclasses")
                    .build())
            .endContainer()
            .withVolumes(
                new VolumeBuilder()
                    .withName("shareclasses")
                    .withNewHostPath("/mnt/disks/share", "DirectoryOrCreate")
                    .build())
  ...
  private Result<InstanceDeployReply> onInstanceDeployRequest(InstanceDeployRequest request) {
    ...
    return Result.of(future)
        .timeoutAfter(1, TimeUnit.MINUTES, () -> new InstanceDeployReply(false, "Timed out", null));
  }
  ...
  private void onInstanceCreatedMessage(InstanceCreatedMessage message) {
    Optional.ofNullable(pendingDeployments.remove(message.instance().deployment().id()))
        .ifPresent(
            future -> future.complete(new InstanceDeployReply(true, null, message.instance())));
  }
}
