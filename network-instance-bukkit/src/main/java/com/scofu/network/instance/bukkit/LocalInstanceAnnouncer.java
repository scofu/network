...
final class LocalInstanceAnnouncer implements Feature {
  ...
  @Inject
  LocalInstanceAnnouncer(MessageQueue messageQueue, MessageFlow messageFlow, Plugin plugin,
      InstanceRepository instanceRepository, LocalInstanceProvider instanceProvider,
      @Named("LocalHost") InetAddress localHost) {
    this.aliveQueue = messageQueue.declareFor(InstanceCreatedMessage.class)
  ...
  public void enable() {
    ...
    instanceProvider.get().thenAcceptAsync(instance -> {
